...
def recv_ecc():
    ...
    while time.time() - last_activity < IDLE_LIMIT_ECC:
        ...
        if t == TYPE_SYN_CHUNK:
            ...
            if declared_parts: log.progress("ECC chunks", len(buf), declared_parts)
            else: log.progress("ECC chunks", parts_seen, parts_seen)
...
def diff_bits_bob(rssi_raw, kept_idx, eps, win, lag):
    ...
    for i in kept_idx:
        j = i + lag
        if not (1 <= j < n):
            # Alice filtered these out; should rarely happen
...
if __name__ == "__main__":
    ...
    # Build Bob's bits aligned to kept_idx with lag
    bitsB = diff_bits_bob(rssi, kept_idx, eps, win, lag)
    # For safety, align length to Alice's advertised total_bits
