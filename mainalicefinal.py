...
# --- Differential "best low-KDR" preset ---
DIFF_EPS_DB  = 2.0  # change to 0.75 for "more bits" preset
DIFF_WIN     = 2      # smoothing window (samples); 0 disables
DIFF_LAG     = +3     # Bob later by 1 (use 0 for no lag)

# ECC (same as your current)
HAM_N        = 15     # block length
DIFF_GUARD_MULT  = 3.0   # guard = ±(GUARD_MULT * eps) → ±6 dB
ECC_CHUNKED  = True
ECC_CHUNK_BYTES = 20
...
TYPE_REPLY      = 0xB2
TYPE_KEEP       = 0xC0
TYPE_THR        = 0xC3   # repurposed to carry (epsilon, win, lag)
TYPE_READY_ECC  = 0xC9
TYPE_SYN        = 0xD0
TYPE_SYN_CHUNK  = 0xD1
TYPE_SYN_END    = 0xD2
...
def diff_quantise_alice(rssi_raw, eps=DIFF_EPS_DB, win=DIFF_WIN, lag=DIFF_LAG):
    # Stage 2 — Differential Quantisation (with guard band)
    ...
    # Guard band: keep only if |Δ| ≥ thr, where thr = GUARD_MULT * eps
    thr = float(eps) * float(DIFF_GUARD_MULT)
...
def send_kept_indices(kept_idx):
    total=len(kept_idx); sent=0; chunks=0; t0=time.perf_counter(); CHUNK=8
    while sent < total:
        ...
        sent += len(chunk); chunks += 1; time.sleep(0.03)
...
def send_diff_params(eps, win, lag):
    # Reuse TYPE_THR to carry (float eps, int16 win, int16 lag)
